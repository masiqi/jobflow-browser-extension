...
begin
  ...
  if opportunity.platform <> 'liepin'
    or opportunity.current_status not in ('draft_ready', 'extracting') then
    raise exception 'draft_ready_required' using errcode = '23514';
  end if;
  if opportunity.current_status = 'extracting' and not exists (
    select 1
    from public.delivery_records d
    where d.opportunity_id = target_opportunity_id
      and d.user_id = auth.uid()
      and d.draft_revision_id = target_draft_revision_id
      and d.draft_sha256 = target_draft_sha256
  ) then
    raise exception 'prior_delivery_identity_required' using errcode = '23514';
  end if;
