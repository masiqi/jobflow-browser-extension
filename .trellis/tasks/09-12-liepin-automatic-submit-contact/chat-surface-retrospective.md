...
For the active debugging phase, the user then approved a deliberate behavior change: when a job-bound final application or greeting control is actually clicked but the page cannot be read back, `automatic_send` treats that component as complete and continues. If only the greeting control is available, the composite application assumption is recorded as `application_greeting_click_assumed_success`; the other cases use `application_click_assumed_success` or `greeting_click_assumed_success`. The content command skips the long read-back wait in this mode, adds a short click grace, and the background records the explicit code. This mode is limited to automatic batches; preflight blockers and reviewed-send remain strict, and the reason states that the result was counted by click rather than independently verified.
