...
## Platform UI Evidence Boundary

Recruitment-platform UI components are another cross-layer contract. A platform
may accept a click immediately, render the editor asynchronously, and append a
sent message in a sibling or virtualized subtree. Treating the input control as
the evidence root, or using a short fixed timeout, can record a real write as
unverified even when the platform eventually shows it.

Before changing a platform adapter:

- [ ] Inspect the platform-owned component bundle or a redacted live DOM to
      identify the actual input, send-control, message-list, and sender/direction
      markers.
- [ ] Separate component discovery timeout from post-write evidence timeout.
- [ ] Bind evidence search to the smallest verified chat container; do not fall
      back to page-wide text matching.
- [ ] Require an outbound marker (for example a sender class or direction
      attribute) before accepting exact message text; identical inbound text is
      not evidence.
- [ ] Match aggregate text as well as leaf text because rich text often spans
      multiple elements.
- [ ] Add a pre-existing-message case so recovery can verify without clicking
      send again.
- [ ] Add a delayed-rendering case and use virtual time to cover the full wait.
