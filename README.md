...
JobFlow 是一个 Chrome Manifest V3 扩展。用户先使用猎聘自带的搜索和筛选器完成粗筛，扩展读取当前页面已经加载的职位卡片，串行打开公开详情页，执行有限且可解释的 JD 规则与 LLM 适配度判断，生成可审核、可编辑、可追溯的招呼语草稿，并允许用户在管理台对单个职位明确确认后执行投递并打招呼。

> 当前开发版本默认只生成草稿。唯一 live 入口是管理台内单职位、两阶段确认的 `reviewed_send`；不支持批量或自动发送，不上传新的简历文件，不读取 Cookie，也不调用招聘平台私有写 API。
...
- 独立扩展管理页负责账号、简历画像、规则、模型、人工复核、职位记录、待发草稿和单职位人工确认发送；
- content script 不再注入大型浮动面板；
- side panel 不执行真实发送，live 操作只存在于管理台的职位详情中。

## 安全边界

以下约束是当前构建的硬边界：

1. 不存在批量、自动、后台或页面打开即发送的 live 模式；
2. 真实动作只有管理台逐条 PREPARE/CONFIRM 两阶段入口；
...
13. 投递和招呼语必须分别取得平台回读证据，点击完成或无报错不是成功；
...
12. 查看、编辑或重新生成草稿；
13. 需要真实投递时先打开对应猎聘详情页，在管理台选择“准备发送”；
14. 检查预检状态后选择“确认真实投递并发送”；
15. 分别查看投递、招呼和整体状态，partial 只补做未验证组件；
16. 必要时对排除记录选择“作为例外继续”。

扫描和草稿生成均不表示已经向招聘方发送或投递；只有 delivery record 的 verified 证据表示对应真实动作成功。
...
猎聘 reviewed-send 默认每日值为 150，允许用户在 1 到 500 之间调整，按 Asia/Shanghai 自然日统计。这个值是产品设置，不是猎聘官方允许量。批量与自动 live 行为仍必须单独立项、评审并人工验收，不能通过当前设置字段解锁。
