...
- 不判断疑似重发，也不合并不同职位 ID；
- 当前扫描快照最多保留 500 条；不再设置“本批上限”，用户勾选的可处理职位会全部加入当前队列；
- 详情标签页串行打开，一次最多一个；
...
- 详情内容脚本在 `document_idle` 后仍等待浏览器 `load` 和两个稳定渲染回合，再开始有限时长的 DOM 提取；发送前预检也会等待延迟出现的职位绑定控件；
...
- side panel 负责扫描、预览、批次控制、进度和最近记录；当前结果旁的“刷新状态”只重新读取扩展状态，不重新扫描页面；
- 职位列表提供“全选可处理职位”，用户勾选的职位会全部加入当前队列；
- 页面动作只在可见、职位绑定的控件上执行：先聚焦，再调用控件原生 `click()`；这改善页面兼容性，但不伪造可信事件，也不绕过猎聘安全验证；
...
- JobFlow 只维护一个 Chrome 扩展，通过平台适配器复用 side panel、后台队列、Supabase 数据、模型、额度和证据链；
- 猎聘之后的下一个平台确定为智联招聘；首个增量只做当前页面职位扫描、详情保存、JD 规则和草稿生成；
- 智联招聘的 reviewed-send/automatic-send 必须在只读适配器和合成 fixture 稳定后另行验收；BOSS 直聘暂缓，不为规避其风控开发特殊插件或指纹逻辑；
- 平台差异只放在路由、职位 ID、列表/详情 DOM、投递控件和风控分类适配器中，不能复制一整套插件或绕过平台安全验证。
...
9. 勾选需要处理的职位（可使用“全选可处理职位”）并点击开始生成；所有勾选的职位都会加入当前队列；
...
- 详情生命周期诊断：详情页如果 `load` 未在 10 秒内完成，会在 content console 记录 `[JobFlow:detail-lifecycle]` 及 `readyState`/`visibilityState`，随后继续有限 DOM 等待；这不是风控绕过或资源拦截；
